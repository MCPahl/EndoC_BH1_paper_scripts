...
intersection_spread = intersection_spread[intersection_spread$celltype %in% c("hMSC_osteoblasts", "hMSC_Adipocytes")==F,]
...
tpm_mean = tpm_mean[tpm_mean$celltype %in% c("hMSC_Adipocytes","hMSC_osteoblasts")==F, ]
tpm_mean = left_join(tpm_mean, ocr_connected_genes)

tpm_mean$class[is.na(tpm_mean$class)] = "not_contacted"

pdf("plots/contacted_genes_expression_hMSC_removed.pdf", useDingbats=FALSE)
...
key_df = data.frame(tissue = c( "Adipose...Subcutaneous", "Adipose...Visceral..Omentum.","Adipose...Subcutaneous", "Adipose...Visceral..Omentum.", "Liver", "Pancreas"), celltype = c("SGBS_Diff","SGBS_Diff", "SGBS_Undiff", "SGBS_Undiff", "HEPG2", "EndoC_BH1"))
...
pdf("plots/gtex_connected_genes_expression_hMSC_removed.pdf", useDingbats=FALSE)
